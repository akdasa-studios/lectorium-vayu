...
from lectorium.tasks.files import archive_file, upload_file_to_storage
...
from lectorium.tasks.audio import denoise_audio, normalize_audio

from lectorium.tasks.transcript import (
    extract_transcript,
    proofread_transcript,
    save_transcript,
    select_transcript_to_translate,
...
def process_track():
    ...
    def get_file_to_process(dag_run: DagRun, ti: TaskInstance) -> FileToProcess:
    ...
    with TaskGroup(group_id="metadata"):
        with TaskGroup(group_id="extract"):
            file_metadata = extract_metadata_from_file_name(
                file["file_path"], "ru"
            )  # TODO: lang_source)
    ...
    with TaskGroup(group_id="transcript") as group_transcript:
        with TaskGroup(group_id="generate"):
            transcript_raw = extract_transcript.partial(file_url=file_audio_url).expand(
                language=lang_source
            )
            transcript_edited = proofread_transcript.partial(metadata=metadata).expand(
                transcript=transcript_raw
            )

        with TaskGroup(group_id="translate"):
            transcript_best = select_transcript_to_translate(transcript_edited)
            transcript_translated = translate_transcript.partial(
                transcript=transcript_best
            ).expand(language=lang_target)

        transcript_all = transcript_translated.concat(transcript_edited)
        save_transcript.partial(track_id=track_id, env=env).expand(
    ...
    with TaskGroup(group_id="search_index") as group_search_index:
        ...
        index_words = generate_search_index(
            index_raw_data, languages=lang_source
        )  # TODO: index_raw_data may contain multiple languages

        save_search_index.partial(track_id=track_id, env=env).expand(word=index_words)
